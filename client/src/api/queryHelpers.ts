...
export const poster = async <T, B = unknown>(url: string, body: B): Promise<T> => { // B para el tipado opcional del body
  const { data } = await axiosClient.post<T>(url, body);
  return data;
}

export const putter = async <T, B = unknown>(url: string, body: B): Promise<T> => {
